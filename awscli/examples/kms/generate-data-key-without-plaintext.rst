...
* Base64-decodes the output
    ...
    All fields in an AWS CLI response are Base64 encoded string. Before saving the encrypted data key in a file, this command uses the ``base64`` utility to decode it into binary data. In Windows, use ``certutil`` or a PowerShell cmdlet.
